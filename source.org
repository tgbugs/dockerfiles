...
unset _refresh _repos _repackage _resnap _live_rebuild _nopkgbldr _sync_gentoo
...
To run as a deamon use =-d= option in then =docker ps= to get the
...
#+end_src
# _g=$(basename ~/ni/dev/client*protocols.io.json)
# -v ~/ni/dev/${_g}:/var/lib/sparc/.config/orthauth/client_secret_protocols.io.json \
# -v ~/.config/sparcur/config.yaml:/var/lib/sparc/.config/sparcur/config.yaml \

build helper to work around missing prompt toolkit dep without rebuilding the world
#+begin_src screen
docker run \
--volumes-from local-portage-snap \
-v ~/files/binpkgs/multi:/var/cache/binpkgs \
