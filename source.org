...
#+property: header-args:screen :session org-session :cmd sh
...
# image builds
<<&musl-build-kg-release>>
<<&musl-build-kg-release-user>>
<<&musl-build-kg-dev>>
<<&musl-build-kg-dev-user>>
}
run
# package host
...
*** etc
**** repos.conf
#+begin_src conf :tangle ./musl/profile/docker-profile.conf
[docker-profile]
...
*** profiles
#+begin_src conf :tangle ./musl/profile/docker-profile/metadata/layout.conf
masters = gentoo
profile-formats = portage-2
#+end_src

#+begin_src conf :tangle ./musl/profile/docker-profile/profiles/repo_name
docker-profile
#+end_src

# NOTE that tgbugs/musl/x is listed here but not populated until later
#+begin_src conf :tangle ./musl/profile/docker-profile/profiles/profiles.desc
...
#+begin_src conf :tangle ./musl/profile/docker-profile/profiles/tgbugs/musl/packages
*dev-vcs/git
*app-eselect/eselect-repository
#+end_src
**** parent
#+begin_src conf :tangle ./musl/profile/docker-profile/profiles/tgbugs/musl/parent
gentoo:default/linux/amd64/17.0/musl/hardened
#+end_src
**** make.defaults
# FIXME for some reason this does not actually disable all the flags
#+begin_src conf :tangle ./musl/profile/docker-profile/profiles/tgbugs/musl/make.defaults :tangle no
VIDEO_CARDS=""
#+end_src
**** mask
#+begin_src conf :tangle ./musl/profile/docker-profile/profiles/tgbugs/musl/package.mask
# insurance
dev-lang/rust
dev-lang/rust-bin
...
#+end_src
**** unmask
#+begin_src conf :tangle ./musl/profile/docker-profile/profiles/tgbugs/musl/package.unmask
# gtknor
<gnome-base/librsvg-2.41
#+end_src
**** accept_keywords
#+begin_src conf :tangle ./musl/profile/docker-profile/profiles/tgbugs/musl/package.accept_keywords
...
#+end_src
**** package.use
#+begin_src conf :tangle ./musl/profile/docker-profile/profiles/tgbugs/musl/package.use
# musl performance
...
#+end_src
**** use.mask
#+begin_src conf :tangle ./musl/profile/docker-profile/profiles/tgbugs/musl/use.mask
# reduce deps
perl
gtk
cups
...
--file musl/profile-x/Dockerfile musl/profile-x
#+end_src

*** file
#+begin_src dockerfile :noweb yes :tangle ./musl/profile-x/Dockerfile
FROM busybox:latest as builder
...
*** profiles
**** parent
#+begin_src conf :tangle ./musl/profile-x/docker-profile/profiles/tgbugs/musl/x/parent
..
#+end_src
**** package.use
#+begin_src conf :tangle ./musl/profile-x/docker-profile/profiles/tgbugs/musl/x/package.use
...
#+begin_src conf :tangle ./musl/profile-x/docker-profile/profiles/tgbugs/musl/x/package.mask
<media-libs/mesa-21.2
#+end_src
**** accept_keywords
needed as a stop gap until there is a stable version >=media-libs/mesa-21.2
#+begin_src conf :tangle ./musl/profile-x/docker-profile/profiles/tgbugs/musl/x/package.accept_keywords
...
--file musl/profile-nox/Dockerfile musl/profile-nox
#+end_src

*** file
#+begin_src dockerfile :noweb yes :tangle ./musl/profile-nox/Dockerfile
FROM busybox:latest as builder
...
*** profiles
**** parent
#+begin_src conf :tangle ./musl/profile-nox/docker-profile/profiles/tgbugs/musl/nox/parent
..
#+end_src

**** package.use
#+begin_src conf :tangle ./musl/profile-nox/docker-profile/profiles/tgbugs/musl/nox/package.use
...
#+begin_src xml :tangle ./musl/portage-maven/settings.xml :mkdirp t
...
--file musl/eselect-repo/Dockerfile .
#+end_src
...
COPY --from=tgbugs/musl:profile-x / /

RUN \
eselect profile set docker-profile:tgbugs/musl/x
...
# install freetype without harfbuzz to avoid the circular dependency
RUN --mount=from=tgbugs/repos:latest,source=/var/db/repos,target=/var/db/repos,rw \
USE=-harfbuzz emerge -1q \
   --getbinpkg \
   media-libs/freetype \
<<&archive-or-rm>>

# fontconfig is also a mess so we roll it in here
RUN --mount=from=tgbugs/repos:latest,source=/var/db/repos,target=/var/db/repos,rw \
emerge -j4 -1q \
   --getbinpkg \
   media-libs/freetype \
...
#+end_src
# --binpkg-respect-use=n freetype/harfbuzz circularity issues sigh devs

*** world
#+name: world-package-builder
#+begin_src conf :noweb yes :tangle ./musl/package-builder/world
...
#+begin_src conf :noweb yes :tangle ./musl/package-builder-nox/world
<<world-debug>>
<<world-emacs>>
<<world-dynapad-base>>
<<world-python>>
#+end_src
# dev-lisp/sbcl  # requires a crossdev environment for this to work
# dev-lisp/clozurecl
# dev-lisp/clisp
...
--file musl/package-builder-nox/Dockerfile .
#+end_src
...
--file musl/binpkg-only/Dockerfile .
#+end_src
...
--file musl/binpkg-only-nox/Dockerfile .
#+end_src
...
#+begin_src conf :noweb yes :tangle ./musl/kg-release/world
<<world-emacs>>
dev-libs/nss
dev-java/icedtea-bin::musl
...
--build-arg KUID=${UID} \
--file musl/kg-release-user/Dockerfile .
#+end_src
...
*** world
#+name: world-kg-dev
#+begin_src conf :noweb yes :tangle ./musl/kg-dev/world
<<world-kg-release>>
dev-scheme/racket
net-libs/nodejs
app-arch/zip
