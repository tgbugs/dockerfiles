# -*- orgstrap-cypher: sha256; orgstrap-norm-func-name: orgstrap-norm-func--dprp-1-0; orgstrap-block-checksum: ca88aa7f3d97b58f02f9b144f68ec8831529dc92d10011f5e5f39df1a888cf9b; -*-
...
[ -z $RESNAP ] || gnu-builder-resnap || return $?
...
set -e -o pipefail; \
cd etc/portage/env; \
cpvs=$(cat ../package.accept_keywords/profile.haskell); \
for cpv in ${cpvs}; do \
if [ -f ${cpv} ]; then \
    cat ghc-rtsopts >> ${cpv}; \
else \
...
<<&docker-run-prefix>>
emerge --color=y --with-bdeps=y --onlydeps --onlydeps-with-rdeps=n --onlydeps-with-ideps=y -q --keep-going -uDN ${targets_nogruser}
<<&docker-run-suffix>>

echo only targets not acct-group
<<&docker-run-prefix>>
emerge --color=y --with-bdeps=y --nodeps -1 -q --keep-going -u ${targets_nogruser}
<<&docker-run-suffix>>
