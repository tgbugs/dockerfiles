# -*- orgstrap-cypher: sha256; orgstrap-norm-func-name: orgstrap-norm-func--dprp-1-0; orgstrap-block-checksum: c49d7aafbde863f449ab36a5482d52a5715bd4b0d9b8d130aeff899f3199b68e; -*-
...
<<&gnu-run-cross-musl-sbcl>>
<<&gnu-build-musl-cross-sbcl>>
}
...
<<&builder-args>>
${_builder} \
sh -c 'set -o pipefail; emerge --color=n --with-bdeps=y -q -uDN -p @docker | grep "\(^\[ebuild\|acct-\)" | cut -b17- | awk '"'"'{ print "="$1 }'"'")
...
#+begin_src bash
gnu-builder-run x86_64-pc-linux-musl-emerge -q -j4 --keep-going sys-libs/musl
gnu-builder-run x86_64-gentoo-linux-musl-emerge -q -j4 --keep-going sys-libs/musl

# TODO in bootstrap maybe? or something else to replace the build annoyances?
docker run \
...
sh -c \
"
mkdir -p /usr/x86_64-pc-linux-musl/etc/portage/package.use/99-sbcl;\
...
mkdir -p /usr/x86_64-gentoo-linux-musl/etc/portage/package.use/99-sbcl;\
...
gnu-builder-run x86_64-gentoo-linux-musl-emerge -q -j4 --keep-going --usepkg dev-lisp/sbcl
#+end_src

Then inside =builder-run=
#+begin_src bash
    ...
    (let ((no-screen    (and build (member "--no-screen" argv)))
      ...
      (let ((path-sparcron-sparcur-config (or cli-ssc path-sparcron-sparcur-config ""))
        ...
        (when build
          ...
          (fix-ocbe-source)
          (execute-src-block "workflow-cli"))
