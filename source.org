# -*- orgstrap-cypher: sha256; orgstrap-norm-func-name: orgstrap-norm-func--dprp-1-0; orgstrap-block-checksum: c5017365b9ada934b784a21950f93b0c993ce1d34dbc6e76276c2b92a677cfc8; -*-
...
#+begin_src bash :noweb yes :results none :var _use_podman=(if use-podman "1" "")
# bash-safe-block
...
for stage3 in {amd64-hardened-openrc,amd64-musl-hardened}; do
    ...
    docker run \
    -v <<&host-binpkgs-root-path()>>/<<&host-binpkgs-repo-name()>>:/var/cache/binpkgs \
...
curl --fail --head http://localhost:<<&host-binpkgs-port()>>/${_binpkgs_repo_name}/Packages || {
    _error=$?
...
#+name: &builder-vars
#+begin_src bash :noweb yes
_path_dockerfiles=${_in_path_dockerfiles:-${PWD}}
_path_binpkgs_root=${_in_path_root_binpkgs:-<<&host-binpkgs-root-path()>>}
...
--rm \
${1} \
/bin/sh -c 'set -o pipefail; { emerge --info 2>&1 | grep "Unable to parse profile" && { echo "bad profile, if using podman did you container init?"; exit 1; } } || _qpvar=$(/tmp/quickpkg-new); [ -z "${_qpvar}" ] || quickpkg ${_qpvar}'
}
#+end_src
...
(defun build-no-gui ()
  ;; FIXME unfortunately there is an issue when trying to `server-start'
  ;; from an emacs process started in batch mode so instead we have to
  ;; start a new graphical emacs and find file with cli options
  ' ;_; yunowork
  (let* ((_ (require 'server))
         (server-name "dockerfiles-server"))
    (server-start) ; need some way to connect to view the other buffer
    (message "server started")
    (execute-src-block "bash-async-package-server-cli")
    (sleep-for 0.1)
    (let ((process (execute-src-block "bash-async-workflow-cli")))
      ;; FIXME TODO need to figure out how to wait for completion
      ;; we can't use non-async because that will block the whole ui
      (while (server-running-p server-name)
        ;; having to wait max 10 seconds after server stop not the worst
        (sleep-for 10))))
            ...
            (if (or no-screen (string= system-type "windows-nt"))
                (let ((emacs-command (expand-file-name invocation-name invocation-directory)))
                  (call-process
                   ...
                   "-no-site-file"
                   "-eval"
                   (let* ((orgstrap-shebang (dedupe-lines "orgstrap-shebang"))
