# -*- orgstrap-cypher: sha256; orgstrap-norm-func-name: orgstrap-norm-func--dprp-1-0; orgstrap-block-checksum: 35bd803c116718f6af5e8ab9f0a909046b64ad227e9dc62dcb1f80071eb53f2d; -*-
...
#+property: header-args:screen :session org-session :cmd sh :noweb no-export :terminal (or)
...
(defun dockerfiles-source-path-check (&optional do-error)
  (let* ((check-paths
          (list
         ...
         (dne-paths
          (cl-loop
           ...
           unless (and (not (null f)) (not (string= f "")) (file-exists-p f))
           collect (cons var f))))
    (when dne-paths
  ...
  (let ((setup (member "setup" argv))
    ...
    (let ((no-screen    (and build (member "--no-screen" argv)))
      ...
      (let ((path-sparcron-sparcur-config (or cli-ssc path-sparcron-sparcur-config ""))
        ...
        (dockerfiles-source-path-check cli-check-paths) ; FIXME TODO can we use --pretend here too?
