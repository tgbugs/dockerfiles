...
(load "~/ni/dev/lisp/trees.lisp")
#+end_src

#+header: :var tag-depends-on-tag=(or tag-depends-on-tag)
#+header: :var docker-file-depends-on-docker-file=(or docker-file-depends-on-docker-file)
#+begin_src lisp
(let ((adj (multi-to-adj tag-depends-on-tag)))
  (apply
   #'concatenate
   ...
   (loop for r in '(nil t)
         collect
         (format-tree nil (adj->nested adj :reverse r) :layout :down :node-formatter #'princ-to-string))))
#+end_src
