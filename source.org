...
rm queries.org sckan.org sckan-tutorial.org
_docsf="https://raw.githubusercontent.com/SciCrunch/sparc-curation/master/docs/"
_docsfsc="${_docsf}sckan/"
curl -O ${_docsf}queries.org
curl -O ${_docsfsc}welcome.org
curl -O ${_docsfsc}tutorial.org
curl -O ${_docsfsc}overview.org
curl -O ${_docsfsc}examples.org
curl -O ${_docsfsc}scratch.org
chmod +x ./queries.org
popd
...
/etc/init.d/scigraph start  # FIXME temp hack around permissions issue related to bad redirect is fixed in repos
/etc/init.d/blazegraph start
su user -c 'emacs -visit ~/welcome.org'
#+end_src

# no longer needed now that we set the root password and have vterm
...
ADD welcome.org welcome.org
ADD tutorial.org tutorial.org
ADD overview.org overview.org
ADD examples.org examples.org
ADD scratch.org scratch.org
...
#+begin_src screen
mkdir -p ./sckan/v0/blazegraph
mkdir -p ./sckan/v0/scigraph
pushd ./sckan/v0
rsync -a /var/lib/blazegraph/blazegraph.jnl blazegraph/
rsync -a /var/lib/blazegraph/prefixes.conf blazegraph/
#rsync /tmp/scigraph-build/sparc-sckan/ scigraph/  # SOON TM
# FIXME EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE
#rsync aws-scigraph-data-scigraph:sparc-data-graph-20210830T154445.zip /tmp/
    pushd scigraph/
    [ -d sparc-data-graph-20210830T154445 ] || {
    cp /tmp/sparc-data-graph-20210830T154445.zip .
    unzip sparc-data-graph-20210830T154445.zip
    ln -s /var/lib/scigraph/sparc-data-graph-20210830T154445 graph
    rm sparc-data-graph-20210830T154445.zip
    }
    popd
~/git/pyontutils/nifstd/scigraph/bin/run-build-services-sparc
cp /tmp/scigraph-build/sparc/services.yaml scigraph/
cp /tmp/scigraph-build/sparc/$(head -n 1 scigraph/services.yaml | cut -b3-) scigraph/
popd

docker build \
--tag tgbugs/sckan:latest \
--build-arg UID=${UID} \
--file sckan/v0/Dockerfile sckan/v0
#+end_src

*** file
# TODO have a version of this that does the builds itself instead of just copying everything in
#+begin_src dockerfile :tangle ./sckan/v0/Dockerfile
FROM busybox:latest as builder
WORKDIR /build

ADD blazegraph/ /build/var/lib/blazegraph
ADD scigraph/ /build/var/lib/scigraph

RUN \
chown -R 834:834 /build/var/lib/blazegraph \
;  chown -R 835:835 /build/var/lib/scigraph
