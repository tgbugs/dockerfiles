...
# FIXME rsync hangs forever in this build environment?
# FIXME as is with the mv and cp this is extremely broken due to docker braindeath
# FIXME BUT SOMEHOW move the COPY --from above and trying emaint sync also hangs forever
# but not after the mv cp crazyness !?
RUN --mount=from=gentoo/portage:latest,source=/var/db/repos/gentoo,target=/var/db/repos/gentoo,rw \
   test -z $SYNC_GENTOO || \
 #mv /var/db/repos /var/db/repos-sigh; \
 #cp -a /var/db/repos-sigh /var/db/repos; \
