# -*- orgstrap-cypher: sha256; orgstrap-norm-func-name: orgstrap-norm-func--dprp-1-0; orgstrap-block-checksum: 1794ce17709c87e5ace68680ce838aa9b677c0e9f0872b3e42a3605c08762664; -*-
...
(defun run-setup ()
  ...
  (let ((bp path-binpkgs-root)
    ...
    (when missing-configs
      ;; even if we can't create /etc/docker/daemon.json we can warn if
      ;; it is missing (for now)
      (error "You have not finished setup %S. Please see the Setup section." missing-configs))))
