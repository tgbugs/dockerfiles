...
# FIXME this should be run if it isn't up
pushd ~/files/binpkgs
...
<<&musl-build-profile-static-x>> || return $?;
  <<&musl-build-eselect-repo>> || return $?;
    ...
    docker rm local-repos-snap; # ok if this fails due to the repo not existing
...
# TODO make volumes if they don't already exist
...
# you can add single packages to build after -uDN for testing
...
-v ~/files/binpkgs/musl-x:/var/cache/binpkgs \
-v /mnt/str/portage/distfiles:/var/cache/distfiles \
-it tgbugs/musl:package-builder-snap \
emerge --color=y -j4 -q --keep-going --usepkg=n \
@live-rebuild
...
#+begin_src screen
docker run \
-v /var/lib/blazegraph:/tmp/blazegraph \
-v /tmp/.X11-unix:/tmp/.X11-unix \
-e DISPLAY=$DISPLAY \
...
#+begin_src bash :tangle ./musl/kg-dev-user/entrypoint.sh
