...
set -e -o pipefail; \
cd etc/portage/env; \
cpvs=$(cat ../package.accept_keywords/profile.haskell <(cat ../package.unmask/profile | grep haskell | awk -F':' '{ print $1 }')); \
for cpv in ${cpvs}; do \
if [ -f ${cpv} ]; then \
    cat ghc-rtsopts >> ${cpv}; \
else \
...
<<&docker-run-prefix>>
sh -c "source /dockerfiles/bin/workflow-funs.sh; batch-emerge-nodeps ${targets_nogruser}"
<<&docker-run-suffix>>
