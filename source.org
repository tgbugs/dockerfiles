...
#+end_src
***** dev-haskell/pandoc
#+begin_src ./docker-profile/base/pandoc.env
GHCRTS=-M5G
#+end_src
...
_builder=${_tm_pbs}
<<&docker-run-prefix>>
sh -c 'targs=$(set -o pipefail; export ACCEPT_KEYWORDS="**"; emerge -p --nodeps @meta-live | { grep "^\[ebuild" || true; } | cut -b17- | awk '"'"'{ print "="$1 }'"'"'); [ "${targs}" ] && { ACCEPT_KEYWORDS="**" emerge --buildpkgonly --nodeps ${targs}; }'
<<&docker-run-suffix>>
}
