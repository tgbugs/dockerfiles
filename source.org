...
# FIXME probably rm -r ./musl to prevent stale/moved files
...
<<&musl-build-profile-x>> || return $?;
  <<&musl-build-eselect-repository>> || return $?;
    [ -z $REPOS ] || \
...
#+begin_src conf :tangle ./musl/profile/local/profiles/tgbugs/musl/packages
*dev-vcs/git
...
#+begin_src conf :tangle ./musl/profile/local/profiles/tgbugs/musl/make.defaults :tangle no
VIDEO_CARDS=""
#+end_src
**** mask
...
*** x
# FIXME this should be built as a separate image so that it can be
# updated/added for the xorg build without having to rebuild the core
**** parent
#+begin_src conf :tangle ./musl/profile-x/local/profiles/tgbugs/musl/x/parent
..
#+end_src
**** use
#+begin_src conf :tangle ./musl/profile-x/local/profiles/tgbugs/musl/x/package.use
...
RUN --mount=from=tgbugs/repos:latest,source=/var/db/repos,target=/var/db/repos,rw \
USE=-harfbuzz emerge -1q --getbinpkg \
   media-libs/freetype \
<<&archive-or-rm>>
#+end_src
** package-builder
...
# FIXME there is some issue with some indirection between virtual/jdk and icedtea-bin
#+begin_src conf
...
*** build
#+begin_src screen :noweb yes :session org-session
<<&docker-build>>
...
--file musl/kg-base/Dockerfile musl/kg-base
#+end_src

*** file
#+name: &musl/kg-base
#+begin_src dockerfile :tangle ./musl/kg-base/Dockerfile
<<&build-world>>
#+end_src

*** world
#+name: world-kg-base
#+begin_src conf :noweb yes :tangle ./musl/kg-base/world
dev-db/blazegraph-bin
dev-java/scigraph-bin
<<world-emacs>>
#+end_src
