...
#+begin_src bash
./source.org build --refresh --repos --repackage --resnap --live-rebuild
#+end_src
** Build
...
#+begin_src screen
unset _refresh _repos _sync_gentoo _repackage _resnap _live_rebuild _nopkgbldr
_refresh=       # pull base images
_repos=         # pull ebuild repo images
_sync_gentoo=   # run emaint sync for gentoo repo
_repackage=     # run quickpackage on base images  # FIXME conditional on actual change ?
...
curl --fail --head http://localhost:8089/multi/Packages || {
    pushd ~/files/binpkgs
...
if [ -n "${_refresh}" ]; then
    ...
    local STAGE3_LATEST="$(curl --fail --silent "${DIST}/latest-stage3-amd64-musl-hardened.txt" |\
        tail -n 1 | cut -f 1 -d'/' | sed -r 's/(....)(..)(..)T(..)(..)(..)/\1-\2-\3T\4:\5:\6/')"
    local LOCDOC_LATEST="$(docker image inspect gentoo/stage3:amd64-musl-hardened --format '{{.Created}}')"
    ...
    # TODO consider whether this should invoke a repackage
    # TODO related to diffing existing binpkgs to installed
fi
...
app-editors/emacs dynamic-loading gmp jit json threads
...
-v /etc/distcc/hosts.docker:/etc/distcc/hosts \
...
RUN \
emacs -batch -eval \
"(let ((user-init-file (pop argv))) (package-initialize) (while argv (orgstrap-whitelist-file (pop argv))) (ow-use-packages evil undo-tree))" \
...
#+begin_src screen
_f=$(sudo su sparc -c 'basename /var/lib/sparc/.config/pyontutils/sparc-curation-*.json')
_h=protocols-io-api-token-rw.pickle
sudo docker run \
-d \
-v /var/lib/sparc/files:/var/lib/sparc/files \
...
# FIXME temporary fix
RUN \
sed -i 's/need net epmd/need epmd/' /etc/init.d/rabbitmq
...
RUN \
mkdir ~/git \
;  mkdir -p /var/lib/sparc/.config/sparcur \
;  mkdir -p /var/lib/sparc/.config/orthauth \
;  chmod 0700 /var/lib/sparc/.config/orthauth
...
# FIXME somehow pyontutils is still getting installed in site packages >:|
# oh, it is worse, if you don't use . it will create a wheel and stick it in
# site packages WHAT, yay variant behavior depending on cwd vs specified path
# so yeah, can't use pip directly at the top level, have to do the pushd dance
RUN \
pushd git \
&&     pushd idlib \
&&         pip install --user -e . \
...
WORKDIR $HOME/git/sparc-curation/sparcur_internal
...
auth-variables:
  google-api-service-account-file-readonly:
    path: google api saro  # FIXME SIGH get_path nonsense
...
# FIXME rabbitmq has a stupid service dependency setup that
# has a hard dependency on the network but it doesn't actually
# need a network process to be started, even a dummy will do ... sigh
/etc/init.d/rabbitmq start

# testing and ensure that there is an existing cache on first run
su sparc -c 'python sparcron.py'

# prod
#pushd /var/lib/sparc/git/sparc-curation/sparcur_internal/
su sparc -c 'PYTHONBREAKPOINT=0 celery --app sparcron worker --beat --schedule-filename ./sparcur-cron-schedule --loglevel=INFO'
...
(defun make-screen-vars ()
  (apply #'format
         "export _refresh=%s _repos=%s _sync_gentoo=%s _repackage=%s _resnap=%s _live_rebuild=%s _nopkgbldr=%s;"
         (mapcar (lambda (b) (if b 1 ""))
                 (list refresh repos sync-gentoo repackage resnap live-rebuild no-pkg-bldr))))
...
(when noninteractive
  (let ((setup (member "setup" argv))
    ...
    (let ((no-screen    (and build (member "--no-screen" argv)))
          ...
          (repackage    (and build (member "--repackage" argv)))
