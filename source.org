...
# FIXME for some reason this fails to pull in the user packages needed by e.g. redis thus the manual merge above
<<&docker-run-prefix>>
...
RUN \
mv /build/quicklisp /quicklisp

RUN \
sbcl --no-interactive --no-userinit --disasble-debugger \
--load /tmp/boostrap-3.lisp

# FIXME find a better location for this
RUN \
mv /quicklisp ${HOME}/quicklisp

# FIXME HACK workaround for bad prcl assumptions
RUN \
mkdir ${HOME}/code/lisp

RUN \
ln -s ${HOME}/code/lisp/quicklisp ../../quicklisp

RUN \
pushd git \
&&     git clone https://github.com/tgbugs/git-share.git \
&&     git-share/README.org tangle \
