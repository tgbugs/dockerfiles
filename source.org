...
<<&builder-args>>
${_tm_pbs} \
sh -c 'rebuilds=$(live-changed); [ -n "${rebuilds}" ] && { emerge --with-bdeps=y --onlydeps --onlydeps-with-rdeps=n --onlydeps-with-ideps=y --color=y -j4 -q --keep-going ${rebuilds} && emerge --color=y -j4 -q --keep-going --usepkg=n --nodeps ${rebuilds}; }'
CODE=$?
...
# FIXME ideally this should build from a set ... maybe @builder ?
RUN --mount=from=tgbugs/repos:latest,source=/var/db/repos,target=/var/db/repos,rw \
   emerge -j4 -q -uDN app-portage/gentoopm \
   --getbinpkg \
   --exclude sys-process/procps \
|| echo $? > /emerge-fail \
<<&archive-or-rm>>
