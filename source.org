# -*- orgstrap-cypher: sha256; orgstrap-norm-func-name: orgstrap-norm-func--dprp-1-0; orgstrap-block-checksum: 4db4c529f5d1c2287e2413375adf8e608a1a5985951e66dbe4d21fbdd4888835; -*-
...
#+end_src
*** file
XXX implementation moved to [[&file-package-builder-static]].
TODO remove this section probably
...
--build-arg UID=${UID} \
--file musl/kg-dev-user/Dockerfile .
...
RUN \
raco pkg install --no-docs --type git-url https://github.com/zussitarze/qrcode.git \
&& pushd git \
&&     pushd rkdf \
...
&&     pushd protc \
&&         raco pkg install --no-docs --auto --batch protc-lib/ protc-tools-lib/ protc/ protc-tools/ \
&&     popd \
&& popd
...
COPY musl/sbcl-stripped/quicklisp-https-always /build/quicklisp/local-projects/quicklisp-https-always
ADD musl/sbcl-stripped/bootstrap-1.lisp /tmp/bootstrap-1.lisp
ADD musl/sbcl-stripped/bootstrap-2.lisp /tmp/bootstrap-2.lisp
ADD musl/sbcl-stripped/bootstrap-3.lisp /tmp/bootstrap-3.lisp
...
RUN \
sbcl --core /tmp/continue.core --no-userinit --disable-debugger \
...
RUN \
sbcl --non-interactive --no-userinit --disasble-debugger \
...
ADD musl/kg-dev-user/entrypoints /etc/entrypoints
ADD musl/kg-dev-user/entrypoint.sh /etc/entrypoint.sh
     ...
     (flatten-list
      (list
       ...
       (cl-loop
        ...
        collect
        (concat "raco pkg install --no-docs --auto --batch " (if name (concat "--name " name " ") "") rp))
       (cl-loop
        for (pi . _) in py-inst
        collect
        (concat "pushd " pi "
...
local version tag spath tpath patch ipath sighpath
version=${1}
tag=sbcl-${version}
...
sbcl-static-patch 2.3.5
sbcl-static-patch 2.4.3
sbcl-static-patch 2.4.9
cp -a <<&helper-repos()>>/sbcl/patches musl/package-builder
#+end_src
# FIXME currently need to copy these into the musl/package-builder
# tree because everything is sandboxed into that folder and it is not
# straight forward to switch the build directory to be the
# ~/git/dockerfiles
...
COPY bootstrap-3.lisp /bootstrap-3.lisp

RUN --network=none \
["/usr/bin/sbcl", \
"--non-interactive", \
"--no-userinit", "--disable-debugger", \
"--load", "/bootstrap-3.lisp"]
...
;;(delete-file *load-truename*) ; don't delete this
(quit)
#+end_src
# probably don't delete the file and don't quit because I think we
# need this any time we start quicklisp so likely will want a way to
# either register this in clrc or something similar, or just always
# run the first 3 lines via clrc or something
...
#+name: lisp-qlha-asdf
#+begin_src lisp :tangle ./musl/sbcl-stripped/quicklisp-https-always/quicklisp-https-always.asd :mkdirp yes
