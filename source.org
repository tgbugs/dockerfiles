# -*- orgstrap-cypher: sha256; orgstrap-norm-func-name: orgstrap-norm-func--dprp-1-0; orgstrap-block-checksum: 3e3be6347dc4066bf61cee6a627f2ef76fe7f8f6998bd98af5a934dad84c3482; -*-
...
[ -z $RESNAP ] || builder-resnap || return $?
...
[ -z $RESNAP ] || static-builder-resnap || return $?
...
[ -z $RESNAP ] || nox-builder-resnap || return $?
...
EMERGE_DEFAULT_OPTS="--jobs 4 --binpkg-respect-use=y"
...
USE="-harfbuzz" emerge -j4 -1q --usepkg media-libs/freetype
doqp

# have to set -cairo here otherwise harfbuzz -truetype will fail to build
USE="-cairo -truetype" emerge -j4 -1q --usepkg media-libs/harfbuzz
doqp
...
USE="static-libs -harfbuzz" emerge -j4 -1q --usepkg media-libs/freetype
doqp
...
<<&docker-run-prefix>>
emerge --color=y --with-bdeps=y -j4 -q --keep-going --usepkg @builder
<<&docker-run-suffix>>
...
_builder=${_tm_pbs}
<<&docker-run-prefix>>
sh -c 'rebuilds=$(live-changed); if [ -n "${rebuilds}" ]; then { emerge --with-bdeps=y --onlydeps --onlydeps-with-rdeps=n --onlydeps-with-ideps=y --color=y -j4 -q --keep-going ${rebuilds} && emerge --color=y -j4 -q --keep-going --usepkg=n --nodeps ${rebuilds}; } fi'
<<&docker-run-suffix>>
}
...
#+begin_src conf :tangle ./musl/package-builder/sets/live-hrm
...
lives = ec.target_config.sets['live'].getAtoms()  # TODO /etc/portage/sets/live
...
for live in sorted(lives, reverse=True):
    ...
    for atom in sorted(atoms, key=(lambda a: (a, a.build_id)), reverse=True):
        ...
        if atom.version == '9999' and not _9999_done:
            ...
            S = bp['S']
            ebuilds[atom.cp] = key_ebuild, ebuild, egit_dir, egit_version
            cp_data[atom.cp] = (
                egit_dir,
                egit_version,
                #egit_min_clone_type,
...
try:
    ...
    for cp, (efn, ebuild, egit_dir, egit_version) in ebuilds.items():
        cpp = ebuild_path / cp
        ...
        out = p.communicate()
        with open(pathlib.Path(egit_dir) / 'FETCH_HEAD', 'rt') as f:
...
<<&builder-args>>
tgbugs/musl:distcc-snap \
emerge  --color=y --with-bdeps=y -j4 -q --keep-going --usepkg \
sys-devel/distcc \
sys-devel/crossdev
...
-v ${_path_dockerfiles}:/dockerfiles \
--rm tgbugs/musl:updated \
bash -c "pushd /dockerfiles; source bin/sbcl-funs.sh; sbcl-clone-fetch && sbcl-ensure-latest && sbcl-generate-patches; CODE=$?; echo TODO chown on linux?; exit ${CODE}" &&
...
gnu-builder-run sh -c 'source /etc/profile; export USE=system-bootstrap; x86_64-pc-linux-musl-emerge -q -j4 --keep-going dev-lisp/sbcl; unset USE;'
...
(defvar-local path-dockerfiles default-directory)
...
%s %s %s;"
         (append
          ...
          (append
           (list default-directory path-distfiles path-binpkgs-root path-binpkgs path-distcc-hosts path-ssh)
