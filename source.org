...
#+begin_src screen
docker pull gentoo/stage3:hardened

# gnu
docker run \
-v ~/files/binpkgs/gnu:/var/cache/binpkgs \
--rm \
gentoo/stage3:hardened \
quickpkg "virtual/ssh" --include-config y;

# musl*
docker pull gentoo/stage3:amd64-musl-hardened
for binrepo in {musl,musl-x,musl-static-x}; do
docker run \
-v ~/files/binpkgs/${binrepo}:/var/cache/binpkgs \
--rm \
...
unset _refresh _repos _repackage _resnap _live_rebuild
_refresh=
_repos=
_repackage=
_resnap=
...
static-builder-world  # FIXME if this is not run once at the start then something fails above
...
because there is some bug in between.  Ideally we should derive the
binpkgs from the selected profile for the image, taking the image as
the first argument and then the rest of the arguments should be. The
other possibility might be to use the output of =emerge --info=.
Indeed =emerge --info= lists the binary repositories along with their
priority, and we have the names of the repo definitions above.
Another option would be =/etc/portage/binrepos.conf/= so that we don't
have to bother with parsing the output of the info dump.
#+begin_src bash
