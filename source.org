...
## NIF-ontology
<<&musl-build-icedtea>> || return $?; echo mbi;
...
docker commit --change='CMD ["/bin/bash"]' $(docker ps -lq) tgbugs/musl:static-package-builder-snap
}
#+end_src
...
-v ~/files/binpkgs/musl-static-x:/var/cache/binpkgs \
-v /mnt/str/portage/distfiles:/var/cache/distfiles \
-it tgbugs/musl:static-package-builder-snap
...
ARG FEATURES="ebuild-lock"  # scigraph and blazegraph pull in acct-{user,group}
<<&build-world>>
#+end_src
