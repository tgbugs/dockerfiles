# -*- orgstrap-cypher: sha256; orgstrap-norm-func-name: orgstrap-norm-func--dprp-1-0; orgstrap-block-checksum: df9bbb1e370aac50d4a4d6ca988a0ade892a208340022fae8f66a3250c2672b9; -*-
...
pypy3 -m interlex.cli --help  # has side effect of creating user config files
...
popd
INTERLEX_DATABASE=$(python -c 'from interlex.config import auth; print(auth.get("test-database"))') \
pypy3 -m pytest --verbose --color=yes -W ignore /usr/share/interlex/test

#+end_src

#+begin_src conf :tangle ./musl/interlex/test-config.yaml
auth-stores:
  secrets:
    path: ~/.config/orthauth/secrets.sxpr
auth-variables:
  interlex-api-key:
    path: interlex api some-user test
  test-api-user: some-user
  interlex-test-api-key:
    path: interlex api some-user test
  alt-db-host: localhost
#+end_src

#+begin_src conf :tangle ./musl/interlex/test-config-pyontutils.yaml
auth-stores:
  secrets:
    path: ~/.config/orthauth/secrets.sxpr
auth-variables:
