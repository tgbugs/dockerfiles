# -*- orgstrap-cypher: sha256; orgstrap-norm-func-name: orgstrap-norm-func--dprp-1-0; orgstrap-block-checksum: 9998674a93881bf359ea7526a6c361ef128ac96d6fd30b70016127c7f56129c3; -*-
...
#+begin_src conf :tangle ./musl/package-builder/set-live
...
-v ${PWD}/musl/package-builder/set-live:/etc/portage/sets/live \
${_builder} \
...
<<&builder-args>>
${_builder} \
emerge --color=y --nodeps -j4 -q --keep-going @live

}
#+end_src

will likely need to set =EGIT_MIN_CLONE_TYPE= so we can check history
will also likely have to dig into the portage code to figure out how to identify
cases where ~S="${S}/${PN}"~ has been set looks like smart-live-rebuild uses
#+begin_src python
...
for atom in sorted(atoms, key=(lambda a: (a, a.build_id)), reverse=True):
    _cat, name, version, _rev = atom.cpv_split[1]
...
for natom, version, name in sorted(not_live):
...
we might be able to do this for binpkgs without having to install stuff?
or install nodeps or something? dealing with the ebuild is a bit harder

#+begin_src bash
bzless /var/db/pkg/dev-python/pyontutils-9999/environment.bz2
EGIT_VERSION
bzless /var/db/pkg/dev-python/ttlser-9999/environment.bz2
#+end_src
...
"--eval", "(asdf:load-system :quicklisp-https-always :verbose nil)", \
"--quit"]
