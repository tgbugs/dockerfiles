...
set -e -o pipefail; \
cd etc/portage/env; \
cpvs=$(cat ../package.accept_keywords/profile.haskell <(cat ../package.unmask/profile | grep haskell | cut -d: -f1) <(echo dev-haskell/dot2graphml) | sed '#.+//' | sort -u); \
for cpv in ${cpvs}; do \
if [ -f ${cpv} ]; then \
    cat ghc-rtsopts >> ${cpv}; \
else \
...
#+begin_src ebuild :tangle ./musl/package-builder/crun.env
