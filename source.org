# -*- orgstrap-cypher: sha256; orgstrap-norm-func-name: orgstrap-norm-func--dprp-1-0; orgstrap-block-checksum: f544374708f14a05c210b8f8e556dba937f0f4906f836308e84047e80dca3749; -*-
...
for stage3 in {amd64-hardened-openrc,amd64-musl-hardened-openrc}; do
...
#+begin_src screen
unset _refresh _repos _sync_gentoo _update_portage _resnap _live_rebuild _nopkgbldr
_refresh=       # pull base images
_repos=         # pull ebuild repo images
_sync_gentoo=   # run emaint sync for gentoo repo
_update_portage= # run emaint sync for gentoo repo and emerge sys-apps/portage
...
_musl_img_stage3=${_musl_img_stage3:-docker.io/gentoo/stage3:amd64-musl-hardened-openrc}
...
function run-common () {
local REPOS="${_repos}"
local UPDATE_PORTAGE="${_update_portage}"
<<&build-user>> || return $?;
...
* docker-profile
** base
The right way to do this is to create two custom profiles on top of musl-hardened-openrc.
...
ADD ${bp}priority.conf                           etc/portage/priority.conf
ADD ${bp}binrepos-multi.conf                     etc/portage/binrepos.conf/multi.conf
ADD ${bp}package.accept_keywords                 etc/portage/package.accept_keywords/00-profile
ADD ${bp}package.accept_keywords.emacs           etc/portage/package.accept_keywords/00-profile.emacs
ADD ${bp}package.accept_keywords.haskell.gentoo  etc/portage/package.accept_keywords/00-profile.haskell
ADD ${bp}package.accept_keywords.versions        etc/portage/package.accept_keywords/00-profile.versions
ADD ${bp}package.mask                            etc/portage/package.mask/00-profile
ADD ${bp}package.unmask                          etc/portage/package.unmask/00-profile
ADD ${bp}package.unmask.versions                 etc/portage/package.unmask/versions
ADD ${bp}package.use                             etc/portage/package.use/00-profile
ADD ${bp}package.use.pypy                        etc/portage/package.use/00-profile-pypy
ADD ${bp}crun.env                                etc/portage/env/app-containers/crun
...
ADD ${bp}package.env                             etc/portage/package.env/00-profile
# general pataches
ADD ${bp}pdf-tools-link-error.patch              etc/portage/patches/app-emacs/pdf-tools/pdf-tools-link-error.patch
...
ADD ${bp}raptor-2.0.16-ttl-parser.patch          etc/portage/patches/media-libs/raptor/raptor-2.0.16-ttl-parser.patch
ADD ${bp}gtk-no-atk-gtkacc.patch                 etc/portage/patches/x11-libs/gtk+/gtk-no-atk-gtkacc.patch
ADD ${bp}gtk-no-atk-mb.patch                     etc/portage/patches/x11-libs/gtk+/gtk-no-atk-mb.patch
ADD ${bp}gtk-no-atk-mot.patch                    etc/portage/patches/x11-libs/gtk+/gtk-no-atk-mot.patch
ADD ${bp}sbcl-2.6.4-musl-termios.patch           etc/portage/patches/dev-lisp/sbcl-2.6.4/sbcl-2.6.4-musl-termios.patch
#ADD ${bp}generically-cabal.patch                 etc/portage/patches/dev-haskell/generically/generically-cabal.patch
...
set -e -o pipefail; \
cd etc/portage/env; \
cpvs=$(cat ../package.accept_keywords/00-profile.haskell <(cat ../package.unmask/00-profile | grep haskell | cut -d: -f1) <(echo dev-haskell/dot2graphml) | sed 's/#.*//' | sort -u); \
for cpv in ${cpvs}; do \
if [ -f ${cpv} ]; then \
    cat ghc-rtsopts >> ${cpv}; \
else \
...
This works because portage combines sections in multiple files into a single record
so we don't have to modify eselect-repo.conf directly.
#+begin_src conf :tangle ./docker-profile/base/priority.conf
[gnu-elpa]
priority = -1001
...
dev-util/shake::gentoo
dev-util/shellcheck::gentoo
sys-apps/racket-where::racket-overlay
#+end_src

Transient workarounds for specific versions.
#+begin_src conf :tangle ./docker-profile/base/package.accept_keywords.versions
=app-emacs/bind-key-2.4.1::gnu-elpa
=app-emacs/haskell-mode-17.5::gentoo  # 17.4 broken
=app-emacs/use-package-2.4.6::gnu-elpa  # needed by quelpa-use-package ??? must be a bug in the ebuild?
#+end_src
...
#+begin: haskell-9.2-to-9.4-to-9.8-bootstrap
It is now possible to ghcbootstrap =dev-lang/ghc-9.8.4::gentoo=
from =dev-lang/ghc-9.4.8::gentoo= from =dev-lang/ghc-9.2.8::gentoo=
on musl without going back to the alpine package.

The set of restrictions below captures most of the requirements,
however it is incomplete because there are multiple steps where
different versions of hadrian are needed that have different
requirements.

You will need the following to bootstrap =dev-lang/ghc-9.8.4= due to
=dev-haskell/hadrian= inducing a circular dependency if
=dev-haskell/cabal= or =dev-haskell/text= versions are too new.
=dev-haskell/hadrian-9.8.4-r1::gentoo= also fails to build if some
version around ~>=dev-haskell/parsec-3.1.17~ is installed, works with
=dev-haskell/parsec-3.1.15.0-r1::gentoo=, so the ebuild likely needs
to be fixed.

Bootstrapping 9.8 via 9.4 needs a series of accept_keywords an unmasks to build 9.4 and then upgrade and
#+begin_src conf :tangle unmask :tangel no
=dev-lang/ghc-9.4.8::haskell
=dev-haskell/hadrian-9.4.8-r1::haskell  # keep for 9.8 merge
<dev-haskell/cabal-3.10::haskell
=dev-haskell/cabal-syntax-3.8.1.0::haskell
dev-haskell/js-flot::haskell  # keep for 9.8 merge
#+end_src

For 9.4 and 9.8 and then switch back to generic. Also must mask =>dev-lang/ghc-9.6= and merge =hadrian-9.8= otherwise circularity arrives
You will probably have to emerge -C hadrian because it will conflict with itself ...
#+begin_src conf :tangle accept_keywords :tangle no
<dev-haskell/parsec-3.1.17
<dev-haskell/cabal-3.10  # needed to keep version low enough to bootstrap hadrian for ghc-9.8
<dev-haskell/text-2  # needed to keep version low enough to bootstrap hadrian for ghc-9.8
#+end_src

May still be needed during the transition from 9.4 to 9.8.
#+begin_src conf
dev-haskell/cabal::haskell
#+end_src
#+end:
...
I don't think we can blindly accept these using e.g.
app-emacs/*::overlay because it might unmask versions
that are newer than what is in ::gentoo which we don't
want, might be able to check on the fly by comparing
app-emacs in each of the repos.
#+begin_src conf :tangle ./docker-profile/base/package.accept_keywords.emacs
app-emacs/*::melpa  # ok because the whole overlay is masked

app-emacs/auto-complete::gentoo
app-emacs/bison-mode::gentoo
app-emacs/closql::gentoo
app-emacs/compat::gentoo
app-emacs/cond-let::gentoo
app-emacs/elpy::gentoo
app-emacs/emacsql::gentoo
app-emacs/forge::gentoo
app-emacs/ghub::gentoo
app-emacs/hl-todo::gentoo
app-emacs/js2-mode::gentoo
app-emacs/julia-mode::gentoo
app-emacs/julia-repl::gentoo
app-emacs/magit::gentoo
app-emacs/ninja-mode::gentoo
app-emacs/polymode::gentoo
app-emacs/pyvenv::gentoo
app-emacs/rust-mode::gentoo
app-emacs/sly::gentoo
app-emacs/transient::gentoo
app-emacs/ts::gentoo
app-emacs/typescript-mode::gentoo
app-emacs/websocket::gentoo

app-emacs/crdt::gnu-elpa
app-emacs/heap::gnu-elpa
app-emacs/map::gnu-elpa
app-emacs/parsec::gnu-elpa
app-emacs/peg::gnu-elpa
app-emacs/pq::gnu-elpa
app-emacs/tNFA::gnu-elpa
app-emacs/tramp-theme::gnu-elpa
app-emacs/trie::gnu-elpa
app-emacs/vdiff::gnu-elpa

app-emacs/arduino-mode::nongnu-elpa
app-emacs/gnuplot::nongnu-elpa
app-emacs/orgit::nongnu-elpa

app-emacs/ac-slime::melpa-stable
app-emacs/annalist::melpa-stable
app-emacs/bibtex-completion::melpa-stable
app-emacs/clojure-mode-extra-font-locking::melpa-stable
app-emacs/color-theme-modern::melpa-stable
app-emacs/dash-functional::melpa-stable
app-emacs/docopt::melpa-stable
app-emacs/erlang::melpa-stable
app-emacs/eval-sexp-fu::melpa-stable
app-emacs/evil-collection::melpa-stable
app-emacs/evil-org::melpa-stable
app-emacs/fill-column-indicator::melpa-stable
app-emacs/git-link::melpa-stable
app-emacs/helm-bibtex::melpa-stable
app-emacs/highlight-numbers::melpa-stable
app-emacs/hy-mode::melpa-stable
app-emacs/json-mode::melpa-stable  # the one on gnu-elpa is ... different?
app-emacs/json-snatcher::melpa-stable
app-emacs/jupyter::melpa-stable
app-emacs/keychain-environment::melpa-stable
app-emacs/ob-hy::melpa-stable
app-emacs/org-make-toc::melpa-stable
app-emacs/org-ql::melpa-stable
app-emacs/org-ref::melpa-stable
app-emacs/org-super-agenda::melpa-stable
app-emacs/org-sync::melpa-stable
app-emacs/orgstrap::melpa-stable
app-emacs/outorg::melpa-stable
app-emacs/outshine::melpa-stable
app-emacs/ov::melpa-stable
app-emacs/ox-gfm::melpa-stable
app-emacs/ox-pandoc::melpa-stable
app-emacs/parent-mode::melpa-stable
app-emacs/quelpa::melpa-stable
app-emacs/quelpa-use-package::melpa-stable
app-emacs/simple-httpd::melpa-stable
app-emacs/sparql-mode::melpa-stable
#+end_src
...
app-emacs/ac-sly::melpa
app-emacs/cypher-mode::melpa
app-emacs/evil-paredit::melpa
app-emacs/glsl-mode::melpa
app-emacs/glue::melpa
app-emacs/ob-cypher::melpa
app-emacs/scribble-mode::melpa
app-emacs/sly-repl-ansi-color::melpa
app-emacs/ssh-config-mode::melpa
app-emacs/vimrc-mode::melpa
...
Transient workarounds for specific versions.
#+begin_src conf :tangle ./docker-profile/base/package.unmask.versions
=app-emacs/bind-key-2.4.1::gnu-elpa
=app-emacs/use-package-2.4.6::gnu-elpa  # needed by quelpa-use-package ??? must be a bug in the ebuild?
#+end_src
...
Workaround for missing pypy3 support in newer versions of various python libraries.
#+begin_src conf :tangle ./docker-profile/base/package.use.pypy
dev-python/flake8 -python_targets_pypy3_11
dev-python/ipykernel -python_targets_pypy3_11
dev-python/jupyter-core -python_targets_pypy3_11
dev-python/matplotlib-inline -python_targets_pypy3_11
dev-python/mccabe -python_targets_pypy3_11
dev-python/nbconvert -python_targets_pypy3_11
dev-python/nbformat -python_targets_pypy3_11
dev-python/pyflakes -python_targets_pypy3_11
dev-python/stack-data -python_targets_pypy3_11
dev-python/traitlets -python_targets_pypy3_11
#+end_src
...
**** patches
***** app-emacs/pdf-tools
Need to link against liberror.so if it is present (i.e. on musl).
#+begin_src diff :tangle ./docker-profile/base/pdf-tools-link-error.patch
...
+AC_CHECK_LIB([error], [error])
 AC_LANG_PUSH([C])
 AC_COMPILE_IFELSE([AC_LANG_PROGRAM([
         #include <error.h>
#+end_src
***** dev-haskell/generically
This really shouldn't have an upper bound, but this
aligns with upstream changes to work around that.
https://github.com/haskell-compat/generically/commit/b428e075d60a10c358391b8382cc0ef246c5f500
#+begin_src diff :tangle ./docker-profile/base/generically-cabal.patch
--- a/generically.cabal
+++ b/generically.cabal
@@ -33,7 +33,7 @@
...
***** media-libs/raptor
#+begin_src diff :tangle ./docker-profile/base/raptor-2.0.16-ttl-parser.patch
fix turtle parser doubling blank-node list/collection entries in large files

See https://github.com/dajobe/raptor/issues/76

Always cancel the deferred sequence on non-final chunks because it
will be reparsed and the stale copy should not be retained.
...
***** x11-libs/gtk+
avoid dbus dep
# from https://github.com/lah7/gkt3-classic/pull/101/files
# see also https://bugs.gentoo.org/669234
#+begin_src diff :tangle ./docker-profile/base/gtk-no-atk-gtkacc.patch
...
#+begin_src diff :tangle ./docker-profile/base/gtk-no-atk-mb.patch
--- a/meson.build
+++ b/meson.build
@@ -565,7 +565,6 @@
...
#+begin_src diff :tangle ./docker-profile/base/gtk-no-atk-mot.patch
--- a/meson_options.txt
+++ b/meson_options.txt
@@ -47,3 +47,5 @@
 # input modules
...
+
+option('atk_bridge', type: 'boolean', value: false, description: 'enable atk-bridge')
#+end_src
...
EGIT_OVERRIDE_BRANCH_SCIGRAPH_SCIGRAPH=owlapi-5
...
# gtk avoid dbus
x11-libs/gtk+ -accessibility

# gif libxml2 svg required by certain elisp packages
app-editors/emacs dynamic-loading gif gmp json libxml2 threads svg
...
# needed for x and nox for racket minimal pango apparently
...
# cairo needed for inkscape and pdf-tools, png needed for pdf-tools
...
sys-apps/file python  # apparently not set by some other profile as of ~2026-07-20?

# tgbugs-meta
tgbugs-meta/kg-dev-meta nest  # include docker-cli and acct-group/docker for sckan release
#+end_src
**** package.use.mask
...
ADD ${bp}crun-cfg-err.patch                      etc/portage/patches/app-containers/crun-1.21/crun-cfg-err.patch
#+end_src
...
*** patches
**** app-containers/crun
From https://github.com/containers/crun/pull/1887
See also https://bugs.gentoo.org/963641
#+begin_src diff :tangle ./docker-profile/base/crun-cfg-err.patch
commit 1188a679b10d6c4516e2e3728104c9f4c59deb5b
diff --git a/configure.ac b/configure.ac
index 5f39279d..3573b1a5 100644
--- a/configure.ac
+++ b/configure.ac
...
=./autogen.sh= is not rerun and expects aclocal-1.16 which of course we don't have
so force it in the ebuild for now
#+begin_src ebuild :tangle ./docker-profile/base/crun.env
pre_src_configure() { ./autogen.sh; }
#+end_src
...
&& emaint sync --repo tgbugs-overlay \
&& emaint sync --repo gnu-elpa \
&& emaint sync --repo nongnu-elpa \
&& emaint sync --repo melpa-stable \
&& emaint sync --repo melpa
...
# fix ebuilds in place so that we don't have to maintain them elsewhere
RUN \
pushd /var/db/repos/gentoo/x11-libs/gtk+ \
&& sed -i 's/^IUSE="/IUSE="+accessibility /' gtk+-3.*.ebuild \
&& sed -i 's,\(>=app-accessibility.\+\),accessibility? ( \1 ) dev-libs/atk,' gtk+-3.*.ebuild \
&& ebuild "$(ls *.ebuild | tail -n1)" digest

# fix python eclasses to restore pypy3_11 support
ADD pypy3_11-eclass-fix.py /tmp/pypy3_11-eclass-fix.py
RUN \
python /tmp/pypy3_11-eclass-fix.py /var/db/repos/gentoo/eclass/python-utils-r1.eclass

ADD pypy3_11-distutils-fix.patch /tmp/pypy3_11-distutils-fix.patch
RUN \
patch -p2 -i /tmp/pypy3_11-distutils-fix.patch /var/db/repos/gentoo/eclass/distutils-r1.eclass
...
*** patches
**** distutils-r1.eclass
#+begin_src diff :tangle ./repos/pypy3_11-distutils-fix.patch :mkdirp yes
--- a/eclass/distutils-r1.eclass
+++ b/eclass/distutils-r1.eclass
@@ -1250,7 +1250,7 @@ _distutils-r1_find_best_wheel() {
 				fi
 				: $(( minor-- ))
 			done
-			[[ ! ${is_compatible} ]] &&
+			[[ ! ${is_compatible} && ! ${DISTUTILS_EXT} ]] &&
...
Structural, not a context diff.
=--getbinpkgonly= aside, this is the same lesson: upstream reshapes these two
arrays and a patch that matches on surrounding lines breaks whenever they do.
=python3_{14..15}t= plus =python3_{12..15}= became =python3_{12..13}= plus
=python3_{14..15}{t,}= in a single sync, and the repos build failed the moment
a fresh portage was pulled:

: ERROR: process "patch -p2 -i /tmp/python-eclass-pypy3_11-fix.patch ..." exit code: 1

So find the arrays and move the token between them, which survives any
reshaping of their contents.  Idempotent, so a rerun or a rebuilt layer cannot
double add, and it verifies its own work rather than trusting the edit --
=patch= at least failed loudly, and whatever replaces it has to do the same.
#+begin_src python :tangle ./repos/pypy3_11-eclass-fix.py :mkdirp yes
"""Move pypy3_11 from _PYTHON_HISTORICAL_IMPLS to _PYTHON_ALL_IMPLS."""
...
ECLASS = (
    sys.argv[1]
    if len(sys.argv) > 1
    else "/var/db/repos/gentoo/eclass/python-utils-r1.eclass"
)


def fail(msg):
    sys.exit("pypy3_11-eclass-fix: %s" % msg)


def find(text, name):
    """the body of a `name=( ... )' bash array, as (start, end) offsets"""
    m = re.search(r"^%s=\(\n(.*?)^\)$" % re.escape(name), text, re.M | re.S)
    if not m:
        fail("cannot find %s in %s" % (name, ECLASS))
    return m.start(1), m.end(1)


def has_impl(body):
    return re.search(r"(?<![\w-])%s(?![\w-])" % IMPL, body) is not None


def drop_impl(body):
    out = []
    for line in body.split("\n"):
        # only the token, so the rest of a shared line keeps its spacing
        stripped = re.sub(r"(?<![\w-])%s(?![\w-])" % IMPL, "", line)
...
def add_impl(body):
    # first, because the comment says most preferred last
    first = body.split("\n")[0]
...
def main():
    path = pathlib.Path(ECLASS)
    text = path.read_text()

    a0, a1 = find(text, "_PYTHON_ALL_IMPLS")
    h0, h1 = find(text, "_PYTHON_HISTORICAL_IMPLS")

    in_all = has_impl(text[a0:a1])
    in_hist = has_impl(text[h0:h1])

    if in_all and not in_hist:
        print("pypy3_11-eclass-fix: already applied, nothing to do")
        return

    # later block first, so the earlier one's offsets still hold
    if a0 < h0:
        text = text[:h0] + drop_impl(text[h0:h1]) + text[h1:]
        text = text[:a0] + (text[a0:a1] if in_all else add_impl(text[a0:a1])) + text[a1:]
    else:
        text = text[:a0] + (text[a0:a1] if in_all else add_impl(text[a0:a1])) + text[a1:]
        h0, h1 = find(text, "_PYTHON_HISTORICAL_IMPLS")
        text = text[:h0] + drop_impl(text[h0:h1]) + text[h1:]
    ...
    # Verify by letting bash evaluate the arrays, not by re-reading them with
    # the same regex that wrote them -- a check that shares its parser with the
    # edit only confirms the parser is self consistent.  These lists are bash
    # arrays in an eclass, so there is no portage API for them, but bash itself
    # will do the parsing, brace expansion included: python3_{12..13} becomes
    # two entries here exactly as it does for portage.
    for name, want in (("_PYTHON_ALL_IMPLS", True), ("_PYTHON_HISTORICAL_IMPLS", False)):
        decl = re.search(r"^%s=\(\n.*?^\)$" % re.escape(name), path.read_text(), re.M | re.S)
        out = subprocess.run(
            ["bash", "-c", '%s\nprintf "%%s\\n" "${%s[@]}"' % (decl.group(0), name)],
            capture_output=True, text=True,
        )
        if out.returncode != 0:
            fail("bash could not evaluate %s: %s" % (name, out.stderr.strip()))
        got = IMPL in out.stdout.split()
        if got is not want:
            fail("after editing, %s in %s is %s, expected %s"
                 % (IMPL, name, got, want))
    print("pypy3_11-eclass-fix: moved %s to _PYTHON_ALL_IMPLS" % IMPL)
...
ARG IMG_USER_BUILDER=docker.io/gentoo/stage3:amd64-musl-hardened-openrc
...
Derived from [[https://hub.docker.com/r/gentoo/stage3/tags?page=1&ordering=last_updated&name=musl-hardened-openrc][gentoo/stage3:amd64-musl-hardened-openrc]] \\
Further derived from https://ftp-osl.osuosl.org/pub/gentoo/releases/amd64/autobuilds/current-stage3-amd64-musl-hardened-openrc/ \\
...
RUN \
   test -z $UPDATE_PORTAGE \
|| { mv /var/db/repos /var/db/repos-wat; cp -a /var/db/repos-wat /var/db/repos; rm -r /var/db/repos-wat; emaint sync --repo gentoo; }

RUN --mount=from=docker.io/gentoo/portage:latest,source=/var/db/repos,target=/var/db/repos,rw \
   test -z $UPDATE_PORTAGE \
...
|| { echo 'dev-lang/rust mrustc-bootstrap' >> /etc/portage/package.use/99-rust; echo 'dev-python/charset-normalizer -native-extensions' >> /etc/portage/package.use/99-python-wd-40; emerge -uDN -q --getbinpkg @world; } \
<<&archive-or-rm>>
...
&& eselect repository enable haskell gnu-elpa nongnu-elpa melpa-stable melpa \
&& mv /etc/portage/priority.conf /etc/portage/repos.conf/priority.conf
#+end_src
...
have, particularly with pypy3, openjdk, ghc, sbcl, and
...
*** run
This block is no longer needed since the harfbuzz/freetype circular dependency has been fixed.
...
RUN \
echo 'FEATURES="${FEATURES} buildpkg compress-index"' >> /etc/portage/make.conf \
...
This block is no longer needed for harfbuzz/freetype, but might be useful if something similar comes up in the future. For now we no longer tangle it.
#+header: :shebang "#!/usr/bin/env sh"
#+begin_src bash :tangle ./bin/harfbuzz-freetype-sigh.sh :mkdirp yes :tangle no
function doqp () {
local _qpvar
...
RUN \
sed -i '/racket/d' /etc/portage/package.mask/00-profile

RUN \
sed -i '/racket/d' /etc/portage/package.accept_keywords/00-profile
...
RUN --mount=from=tgbugs/repos:latest,source=/var/db/repos,target=/var/db/repos,rw \
export JDKATOM="=$(emerge -p --usepkg=n --getbinpkg=n openjdk | grep openjdk | awk '{ print $4 }')" \
&& emerge -1q \
   --binpkg-changed-deps=y \
   --getbinpkgonly \
   ${JDKATOM} \
# remove openjdk-bin mask for just this bootstrap if openjdk is not present
|| { sed -i "/openjdk-bin/d" /etc/portage/package.mask/00-profile \
...
${_host_features:+--env FEATURES="${_host_features}"} \
${_host_makeopts:+--env MAKEOPTS="${_host_makeopts}"} \
#+end_src
...
<<&builder-args>>
${_builder} \
sh -c 'set -o pipefail; emerge --backtrack=99 --color=n --with-bdeps=y -q -uDN -p @docker | { grep "\(^\[ebuild\|acct-\)" || true; } | cut -b17- | awk '"'"'{ print "="$1 }'"'")
...
<<&docker-run-prefix>>
emerge --color=y -q --keep-going --getbinpkgonly --usepkgonly --nodeps ${targets_ghc}
<<&docker-run-suffix>>
...
sys-devel/distcc
sys-devel/crossdev
dev-lisp/clisp  # XXX temp workaround for https://bugs.gentoo.org/967525
#+end_src
...
RUN \
sed -i 's,-no-site-file,-load ~/.emacs.d/early-init.el,' ./sckan/queries.org \
&& ./sckan/queries.org
...
RUN \
emacs -batch -eval \
"(let ((early-init-file (pop argv)) (user-init-file (pop argv))) (load early-init-file) (load user-init-file) (while argv (let ((f (pop argv))) (orgstrap-whitelist-file f) (kill-buffer (find-file-noselect f)))) (ow-use-packages evil undo-tree))" \
$HOME/.emacs.d/early-init.el $HOME/.emacs.d/init.el sckan/{queries,welcome,examples,scratch,tutorial}.org
...
#+name: elisp-populate-site-gentoo-packages
#+begin_src elisp :tangle ./musl/kg-release-user/early-init.el
(setq package-enable-at-startup nil)
(require 'package)
(package-initialize 'no-activate)
(let* ((ll (or (locate-library "site-gentoo")))
       (site-lisp-directory
       ...
       (cands
        (cl-loop
         for p in load-path
         when (and (string-prefix-p site-lisp-directory p) (string-match-p "/site-lisp/[^/]+$" p))
         append
         (cl-loop
          for e in (directory-files p)
          when (and (string-suffix-p ".el" e) (not (string-suffix-p "-autoloads.el" e)))
          collect (expand-file-name e p)))))
  (cl-remove-if
   #'null
   (cl-loop
    for c in cands
    ...
    (let* ((pkg-desc
           ...
           (pkg (when pkg-desc (package-desc-name pkg-desc))))
      (when pkg-desc
        (setf (package-desc-dir pkg-desc) (file-name-directory c)) ; FIXME redundant
...
*** run
**** An example of building a SCKAN release
#+begin_src bash
# pull
docker pull tgbugs/sckan:latest
...
#+end_src
Release artifacts are deposited in =/tmp/build= and =/tmp/scigraph-build=.
Setting =SCIGRAPH_API= to localhost:9000 uses the image internal SciGraph with sckan-data.
Setting =PYTHONPATH= to include =~/git/pyontutils= is a hack to avoid needing any further configuration.
Data sync and full release requires additional configuration that is beyond the scope of this example.

In the user container shell.
#+begin_src bash
...
Alternately you can use the =docker run= command above with
=--entrypoint /etc/entrypoints/sckan-release.sh=.

You can track of progress from the host by looking in [[/tmp/build/release-*-sckan/]]

You can get a root shell for the container via
#+begin_src bash
...
/etc/init.d/scigraph start
/etc/init.d/blazegraph start
if [ -e /var/run/docker.sock ] ; then groupmod -g $(stat -c '%g' /var/run/docker.sock) -o docker; fi
...
su user -c '~/git/sparc-curation/bin/build-sckan-release'
#+end_src
...
RUN \
emacs --batch --eval \
"(progn (load (pop argv)) (url-handler-mode 1) (find-file (pop argv)) (eval-buffer))" \
...
RUN \
sed -i 's,-no-site-file,-load ~/.emacs.d/early-init.el,' ./git/sparc-curation/docs/queries.org \
...
RUN \
emacs -batch -eval \
"(let ((early-init-file (pop argv)) (user-init-file (pop argv))) (package-initialize) (while argv (orgstrap-whitelist-file (pop argv))))" \
...
#+begin_src elisp :noweb yes :tangle ./musl/kg-dev-user/early-init.el
<<elisp-populate-site-gentoo-packages>>
#+end_src
...
#+begin_src elisp :noweb yes :tangle ./musl/tgbugs-dev-user/early-init.el
<<elisp-populate-site-gentoo-packages>>
#+end_src
...
#--volumes-from local-portage-snap \
#--rm docker.io/gentoo/stage3:amd64-musl-hardened-openrc \
...
emerge -pq --nodeps sbcl 2>/dev/null | grep ebuild | awk '{$1=$1}1' | awk -F/ '{ print $2 }' | awk -F- '{ print $2 }'
}
...
Backport of sbcl f825ab5db65fd7d68bbd8c39e37a8725409cafce to 2.6.4
#+begin_src diff :tangle ./docker-profile/base/sbcl-2.6.4-musl-termios.patch :mkdirp yes
...
# leave empty for now
# <<elisp-populate-site-gentoo-packages>>
#+begin_src elisp :noweb yes :tangle ./musl/sbcl-user/early-init.el
#+end_src
...
; newer versions of ql-https need these loaded first
(load "/build/quicklisp/quicklisp/package.lisp")
(load "/build/quicklisp/quicklisp/deflate.lisp")
(load "/build/quicklisp/quicklisp/dist.lisp")
...
(defvar-local update-portage nil)
...
(defun make-screen-vars ()
  (apply #'format
         "export _use_podman=%s _refresh=%s _repos=%s _sync_gentoo=%s _update_portage=%s _resnap=%s _live_rebuild=%s _nopkgbldr=%s _only_static=%s \\
...
%s %s %s%s%s;"
         (append
          (mapcar (lambda (b) (if b 1 ""))
                  (list use-podman refresh repos sync-gentoo update-portage resnap live-rebuild no-pkg-bldr only-static))
...
(defun fetch-ghc ()
  ...
  (let ((src-984
         (retfile
          'sha256 '9bbf65983a88c8536971f8cda7039c9af8396ec81010bba7d5dc5e1cc528e394
          "https://files.olympiangods.org/ghc-9.8.4-r1-1.gpkg.tar"))
        (src-948
         (retfile
          'sha256 '1c6ebc05b3d442d0b1257dea7d047edc30d5fa909af996e576c4aff0e9d17a44
          "https://files.olympiangods.org/ghc-9.4.8-1.gpkg.tar"))
        (src-928
         (retfile
...
currently only sbcl static uses this but other things might need it in the future"
  ...
  ;; we could make it dependent only on gentoo/stage3:amd64-musl-hardened-openrc
...
(when (or noninteractive no-screen-start-gui)
  (let ((setup (member "setup" argv))
    ...
    (let ((no-screen    (and build (member "--no-screen" argv)))
          ...
          (update-portage (and build (member "--update-portage" argv)))
      ...
      (let ((path-dockerfiles (or cli-path-dockerfiles path-dockerfiles ""))
        ...
        (when (or screen build build-image test setup)
        ...
        (when screen ; after tangle so ./source.org screen tangle can source from bin correctly
          (fix-ocbe-source)
          (let ((scrn-process (execute-src-block "screen-start")))
            (while (eq (process-status scrn-process) 'run)
              (sleep-for 0.1))))
        (when non-tangled
          (do-non-tangled-files))
        (when test
...
Helper block to start screen session.
#+name: screen-start
#+begin_src screen
HISTFILE=~/.org_session_history
[ -f bin/workflow-funs.sh ] && source bin/workflow-funs.sh
#+end_src
