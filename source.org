...
<<&musl-build-profile>>
<<&musl-build-user>>
<<&musl-build-portage-maven>>
<<&musl-build-eselect-repository>>
<<&musl-build-repos>>
<<&musl-build-updated>>
<<&musl-build-updated-user>>
# quickpkg
# package host
...
*** profiles
#+begin_src conf :tangle ./musl/profile/local/repo_name
local
#+end_src
...
# gtk no rust note that this pulls in harfbuzz so you are already on that train if you are using the xserver at all
...
# scigraph
&& echo 'x11-base/xorg-server xvfb' >> /etc/portage/package.use/99-xorg-server
...
#+end_src
The overlays can take up quite a bit of space so it is better to mount
...
emerge -j4 -q -uDN @world \
   --keep-going \
   --exclude sys-process/procps \
;  [[ -n ${ARCHIVE} ]] || rm -r /var/cache/distfiles/* || true
...
*** run
#+begin_src bash
# --volumes-from local-overlay-snap \
function build_package () {
    docker run \
    --volumes-from local-portage-snap \
    -v ~/files/binpkgs:/var/cache/binpkgs \
    tgbugs/musl:builder \
    $@
}
...
#+begin_src dockerfile
RUN \
echo 'PORTAGE_BINHOST="http://host.docker.internal/packages"' >> /etc/portage/make.conf \
